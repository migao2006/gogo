...
import { getStaticBusRouteDirections, searchStaticBusRoutes } from "@/lib/bus-static";
import type {
  BusLiveVehicle,
  BusRouteAlert,
  BusRouteDirection,
...
const directionCache = new Map<string, CachedValue<BusRouteDirection[]>>();
const directionInFlight = new Map<string, Promise<BusRouteDirection[]>>();
const vehicleCache = new Map<string, CachedValue<BusLiveVehicle[]>>();
const vehicleInFlight = new Map<string, Promise<BusLiveVehicle[]>>();
...
export async function searchBusRoutes(
  ...
  const staticResults = await searchStaticBusRoutes(city, query);
...
async function fetchBusRouteDirections(
  city: string,
  routeUid: string
...
export async function getBusRouteDirections(
  ...
  const key = `${city}:${routeUid}`;
  const now = Date.now();
  const cached = directionCache.get(key);

  if (cached && cached.freshUntil > now) return cached.value;

  const existing = directionInFlight.get(key);
  if (existing) return existing;

  const request = (async () => {
    try {
      const staticValue = await getStaticBusRouteDirections(city, routeUid);
      const value = staticValue ?? await fetchBusRouteDirections(city, routeUid);
      const savedAt = Date.now();
      directionCache.set(key, {
        value,
        freshUntil: savedAt + 6 * 60 * 60_000,
        staleUntil: savedAt + 7 * 24 * 60 * 60_000,
      });
      return value;
    } catch (error) {
      ...
      directionInFlight.delete(key);
    }
  })();

  directionInFlight.set(key, request);
  return request;
}
...
async function fetchBusRouteLiveVehicles(
  city: string,
  routeUid: string
...
export async function getBusRouteLiveVehicles(
  ...
  const existing = vehicleInFlight.get(key);
  if (existing) return existing;

  const request = (async () => {
    try {
      const value = await fetchBusRouteLiveVehicles(city, routeUid);
      const savedAt = Date.now();
      vehicleCache.set(key, {
        value,
        freshUntil: savedAt + 120_000,
        staleUntil: savedAt + 15 * 60_000,
      });
      return value;
    } catch (error) {
      if (cached && cached.staleUntil > Date.now()) return cached.value;
      ...
      vehicleInFlight.delete(key);
    }
  })();

  vehicleInFlight.set(key, request);
  return request;
}
