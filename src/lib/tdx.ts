...
const MIN_REQUEST_INTERVAL_MS = Math.max(
  1_500,
  Number(process.env.TDX_MIN_REQUEST_INTERVAL_MS ?? 2_200)
);
...
const RATE_LIMIT_COOLDOWN_MS = Math.max(
  15_000,
  Number(process.env.TDX_RATE_LIMIT_COOLDOWN_MS ?? 45_000)
);
...
async function waitForRequestSlot(path: string): Promise<void> {
...
async function fetchTdxResponse(
  ...
  await waitForRequestSlot(url.pathname);
    ...
    return await fetch(url, {
      ...
      ...(noStore
        ...
        : {
            ...
            next: { revalidate: Math.max(1, revalidateSeconds) },
          }),
    });
  } catch (error) {
    if (error instanceof DOMException && error.name === "AbortError") {
...
export async function tdxFetch<T>(
  ...
  let response = await fetchTdxResponse(url, token, revalidateSeconds);
  ...
  if (response.status === 429) {
    ...
    console.warn("TDX API rate limited", path, detail.slice(0, 160));
    throw new TdxHttpError(
      "TDX 請求過於頻繁",
      429,
      path,
...
export async function firstSuccessfulTdxFetch<T>(
  ...
  for (const attempt of attempts) {
    try {
      ...
      lastError = error;
      if (error instanceof TdxHttpError && error.status === 429) break;
    }
  }
