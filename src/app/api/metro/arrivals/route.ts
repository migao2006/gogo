...
export async function GET(request: NextRequest) {
  ...
  const stationId = normalizeMetroStationId(operatorId, rawStationId);
  if (!stationId) {
  ...
  try {
    ...
    const officialRows = liveBoardRows(liveBoardPayload).filter(
      (row) => liveBoardStationId(row, operatorId) === stationId
    );
    ...
    const estimated = estimateMetroArrivalsFromLiveBoard({
      operatorId,
