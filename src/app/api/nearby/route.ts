...
import { findNearbyStaticStations } from "@/lib/bus-static";
...
async function reverseGeocode(
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: {
        "user-agent": process.env.GEOCODER_USER_AGENT ?? "TDX-Bus-Now/2.2.2",
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!location.city) {
      ...
      try {
        const staticStations = await findNearbyStaticStations(location.city, lat, lon, radius);
        stations = staticStations ?? await fetchBusStops(location.city, lat, lon, radius);
      } catch (error) {
