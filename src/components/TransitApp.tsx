...
const ROUTE_DETAIL_CACHE_KEY = "tdx-bus-route-detail-cache-v222";
const ROUTE_LIVE_CACHE_KEY = "tdx-bus-route-live-cache-v222";
...
export default function TransitApp() {
  ...
  const routeLiveBlockedUntilRef = useRef<Record<string, number>>({});
  ...
  const loadRouteLive = useCallback(
    async (route: BusRouteSearchResult, silent = false) => {
      ...
      const blockedUntil = routeLiveBlockedUntilRef.current[key] ?? 0;
      const cachedLive = safeJson<
        Record<string, { data: BusRouteLiveResult; savedAt: number }>
      >(ROUTE_LIVE_CACHE_KEY, {})[key];

      if (!silent && cachedLive && now - cachedLive.savedAt < 15 * 60_000) {
        setRouteVehicles(cachedLive.data.vehicles ?? []);
        setRouteLiveWarning(cachedLive.data.warning ?? null);
        setRouteLiveUpdatedAt(cachedLive.data.updatedAt);
      }

      if (routeLiveRequestRef.current) return;
      if (silent && now < blockedUntil) return;
      if (silent && now - lastRequested < 150_000) return;
      if (silent && document.visibilityState !== "visible") return;

      routeLiveRequestRef.current = true;
      lastRouteLiveRequestRef.current[key] = now;
      ...
      try {
        ...
        const cache = safeJson<
          Record<string, { data: BusRouteLiveResult; savedAt: number }>
        >(ROUTE_LIVE_CACHE_KEY, {});
        cache[key] = { data, savedAt: Date.now() };
        localStorage.setItem(ROUTE_LIVE_CACHE_KEY, JSON.stringify(cache));

        if (data.unavailable || !data.vehicles?.length) {
          routeLiveBlockedUntilRef.current[key] = Date.now() + 10 * 60_000;
        } else {
          routeLiveBlockedUntilRef.current[key] = 0;
        }
      } catch (requestError) {
        routeLiveBlockedUntilRef.current[key] = Date.now() + 10 * 60_000;
        if (!cachedLive && !silent) {
          setRouteLiveWarning(
            requestError instanceof Error
              ? requestError.message
              : "車輛位置目前未提供"
  ...
  const loadRouteDetail = async (
    ...
    if (!route.routeUid) {
      setRouteDetailError("此路線缺少可查詢的識別碼");
      return;
    }

    const key = `${route.city}:${route.routeUid}`;
    const cache = safeJson<
      Record<string, { directions: BusRouteDirection[]; savedAt: number }>
    >(ROUTE_DETAIL_CACHE_KEY, {});
    const cached = cache[key];
    const routeNow = clock;
    let directions = cached?.directions ?? [];
    const cacheFresh = Boolean(
      cached && routeNow > 0 && routeNow - cached.savedAt < 6 * 60 * 60_000
    );

    const applyDirections = (items: BusRouteDirection[]) => {
      setRouteDirections(items);
      ...
      const matchedDirection = items.find((direction) =>
        direction.stops.some((stop) => stop.stopUid && targetStopUids.has(stop.stopUid))
      );
      setActiveRouteDirection(matchedDirection?.key ?? items[0]?.key ?? null);
    };
    ...
    if (!cacheFresh) {
      try {
        ...
        if (data.directions?.length) {
          ...
          applyDirections(directions);
          cache[key] = { directions, savedAt: routeNow };
          localStorage.setItem(ROUTE_DETAIL_CACHE_KEY, JSON.stringify(cache));
          setRouteDetailError(null);
        } else if (!cached?.directions?.length) {
          setRouteDetailError(data.warning ?? "此路線目前沒有可顯示的站序資料");
        }
      } catch (requestError) {
      ...
      {selected ? (
        <div className="arrival-backdrop" onClick={() => setSelected(null)}>
          <section className="arrival-sheet" role="dialog" aria-modal="true" aria-label={`${selected.name} 公車即時資訊`} onClick={(event) => event.stopPropagation()}>
            ...
            <p className="sheet-note">到站資料由各縣市業者提供給 TDX。即時秒數在手機端倒數，約每 60 秒重新校正；更新失敗時會保留上次成功資料。</p>
          </section>
        </div>
      ) : null}
      ...
      {selectedRoute ? (
        ...
        >
          ...
          >
            ...
            <div className={`route-live-strip ${selectedRouteVehicles.length ? "" : "is-unavailable"}`}>
              <span>
                <Radio size={15} />
                {routeLiveLoading
                  ? "正在更新車輛位置"
                  : selectedRouteVehicles.length
                    ? `${selectedRouteVehicles.length} 輛車在線`
                    : "即時車輛定位未提供"}
              </span>
              <small>
                {routeLiveWarning
                  ? routeLiveWarning
                  : routeLiveUpdatedAt
                    ? `更新 ${formattedTime(routeLiveUpdatedAt)}`
                    : "約每 3 分鐘更新"}
              </small>
            </div>
            ...
            {routeDetailLoading ? (
              <div className="arrival-loading">
                ...
                <strong>正在取得路線站序</strong>
                <small>站序完成後，再背景取得車輛位置…</small>
              </div>
            ) : null}
            {routeDetailError ? (
