...
function normalizeMetroCode(value?: string): string {
  if (!value) return "捷運";
  return value.replace(/^[A-Z]+-/, "").replace(/[^A-Za-z0-9]/g, "").toUpperCase() || "捷運";
}

function metroLineTone(value?: string): string {
  const code = normalizeMetroCode(value);
  if (code.startsWith("BL")) return "blue";
  if (code.startsWith("BR")) return "brown";
  if (code.startsWith("O")) return "orange";
  if (code.startsWith("R")) return "red";
  if (code.startsWith("G")) return "green";
  if (code.startsWith("Y")) return "yellow";
  if (code.startsWith("A")) return "airport";
  return "default";
}

function metroStatusClass(arrival: MetroArrival, now: number): string {
  if (arrival.estimateSeconds === null) return arrival.trainStatus === 3 ? "ended" : "empty";
  const calculatedAt = arrival.calculatedAt ? Date.parse(arrival.calculatedAt) : now;
  const elapsed = Number.isFinite(calculatedAt) ? Math.max(0, Math.floor((now - calculatedAt) / 1_000)) : 0;
  const remaining = Math.max(0, arrival.estimateSeconds - elapsed);
  return remaining <= 60 ? "soon" : "normal";
}
...
export default function TransitApp() {
  ...
  const [metroDetailTab, setMetroDetailTab] = useState<"arrivals" | "station">("arrivals");
  const [lastTrainOpen, setLastTrainOpen] = useState(false);
  ...
  const loadArrivals = useCallback(async (station: TransitStation, silent = false) => {
    setSelected(station);
    if (!silent) {
      setMetroDetailTab("arrivals");
      setLastTrainOpen(false);
    ...
    try {
      if (station.mode === "bus") {
        ...
        const params = new URLSearchParams({
          ...
          stationIds: (station.stationIds?.length ? station.stationIds : [station.stationId ?? station.uid]).join(","),
        });
      ...
      {selected ? (
        <div className="arrival-backdrop" onClick={() => setSelected(null)}>
          <section className="arrival-sheet" role="dialog" aria-modal="true" aria-label={`${selected.name} 即時資訊`} onClick={(event) => event.stopPropagation()}>
            ...
            <div className={`arrival-header ${selected.mode === "metro" ? "arrival-header--metro" : ""}`}>
              {selected.mode === "metro" ? (
                <div className="metro-station-heading">
                  <div className="metro-station-codes" aria-label="捷運路線代碼">
                    {(selected.lineIds?.length ? selected.lineIds : [selected.lineId ?? "捷運"]).map((lineId) => (
                      <span
                        key={lineId}
                        className={`metro-code metro-code--${metroLineTone(lineId)}`}
                      >
                        {normalizeMetroCode(lineId)}
                      </span>
                    ))}
                  </div>
                  <div>
            ...
            {selected.mode === "metro" ? (
              <div className="metro-tabs" role="tablist" aria-label="捷運站資訊分頁">
                <button
                  type="button"
                  role="tab"
                  aria-selected={metroDetailTab === "arrivals"}
                  className={metroDetailTab === "arrivals" ? "active" : ""}
                  onClick={() => setMetroDetailTab("arrivals")}
                >
                  即時到站
                </button>
                <button
                  type="button"
                  role="tab"
                  aria-selected={metroDetailTab === "station"}
                  className={metroDetailTab === "station" ? "active" : ""}
                  onClick={() => setMetroDetailTab("station")}
                >
                  車站資訊
                </button>
              </div>
            ...
            {!arrivalsLoading && selected.mode === "metro" && metroDetailTab === "arrivals" ? (
              <section className="metro-board">
                <header className="metro-section-heading">
                  <div>
                    <h3>進站動態</h3>
                    <span>{metroSourceSummary ? `${metroSourceSummary}・每 15 秒校正` : "即時資料每 15 秒校正"}</span>
                  </div>
                  <button
                    type="button"
                    className="metro-refresh"
                    aria-label="重新整理捷運到站資訊"
                    onClick={() => void loadArrivals(selected)}
                  >
                    <RefreshCw size={18} />
                  </button>
                </header>

                <div className="metro-board-list">
                  {metroArrivals
                    .filter((arrival) => arrival.trainStatus !== 3)
                    .map((arrival, index) => (
                      <div key={`${arrival.trainNumber ?? arrival.destination}-${index}`} className="metro-board-row">
                        <span className={`metro-eta-pill metro-eta-pill--${metroStatusClass(arrival, clock)}`}>
                          {metroEtaLabel(arrival, clock)}
                        </span>
                        <span className="metro-to">往</span>
                        <span className={`metro-code metro-code--${metroLineTone(arrival.destinationStationId ?? arrival.lineId)}`}>
                          {normalizeMetroCode(arrival.destinationStationId ?? arrival.lineId)}
                        </span>
                        <div className="metro-destination">
                  ...
                  {!metroArrivals.filter((arrival) => arrival.trainStatus !== 3).length && !arrivalMessage ? (
                    <div className="metro-empty-row">目前沒有可顯示的進站動態。</div>
                  ) : null}
                </div>

                <button
                  type="button"
                  className="metro-collapse-button"
                  aria-expanded={lastTrainOpen}
                  onClick={() => setLastTrainOpen((current) => !current)}
                >
                  <span>末班車資訊</span>
                  <span className={lastTrainOpen ? "open" : ""}>⌃</span>
                </button>

                {lastTrainOpen ? (
                  <div className="metro-last-train">
                    {metroArrivals.filter((arrival) => arrival.trainStatus === 3).length ? (
                      metroArrivals
                        .filter((arrival) => arrival.trainStatus === 3)
                        .map((arrival, index) => (
                          <div key={`${arrival.destination}-last-${index}`} className="metro-board-row metro-board-row--last">
                            <span className="metro-eta-pill metro-eta-pill--ended">末班已過</span>
                            <span className="metro-to">往</span>
                            <span className={`metro-code metro-code--${metroLineTone(arrival.destinationStationId ?? arrival.lineId)}`}>
                              {normalizeMetroCode(arrival.destinationStationId ?? arrival.lineId)}
                            </span>
                            <div className="metro-destination"><strong>{arrival.destination}</strong></div>
                          </div>
                        ))
                    ) : (
                      <p>TDX 目前未提供此站可顯示的末班車狀態。</p>
                    )}
                  </div>
                ) : null}
              </section>
            ...
            {!arrivalsLoading && selected.mode === "metro" && metroDetailTab === "station" ? (
              <section className="metro-station-info">
                <div className="metro-info-card">
                  <span>營運單位</span>
                  <strong>{selected.operatorId ?? "捷運營運單位未提供"}</strong>
                </div>
                <div className="metro-info-card">
                  <span>路線</span>
                  <div className="metro-inline-codes">
                    {(selected.lineIds?.length ? selected.lineIds : [selected.lineId ?? "捷運"]).map((lineId) => (
                      <span key={lineId} className={`metro-code metro-code--${metroLineTone(lineId)}`}>
                        {normalizeMetroCode(lineId)}
                      </span>
                    ))}
                  </div>
                ...
                <div className="metro-info-card">
                  <span>車站代碼</span>
                  <strong>{(selected.stationIds?.length ? selected.stationIds : [selected.stationId ?? selected.uid]).join("、")}</strong>
                </div>
                <div className="metro-info-card">
                  <span>地址</span>
                  <strong>{selected.address ?? "TDX 未提供車站地址"}</strong>
                </div>
                <div className="metro-info-card">
                  <span>距離搜尋中心</span>
                  <strong>{distanceLabel(selected.distanceMeters)}</strong>
                </div>
              </section>
            ) : null}
