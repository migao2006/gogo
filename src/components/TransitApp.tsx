...
export default function TransitApp() {
  ...
  const loadRouteLive = useCallback(async (route: BusRouteSearchResult, silent = false) => {
    ...
    try {
      ...
      if (!silent) {
        setRouteLiveWarning(
          requestError instanceof Error ? requestError.message : "車輛位置暫時無法取得"
        );
      }
    } finally {
    ...
    const checkReminders = async () => {
      ...
      for (const stationReminders of [...byStation.values()].slice(0, 5)) {
        ...
        try {
          ...
          const data = (await response.json()) as BusArrivalResult;
          ...
          // Reminder checks are best effort and should not interrupt the main UI.
        }
      }
